...
export function hasCuratedVisualKeyword(word: string): boolean {
  ...
  const phrase = CURATED_VISUAL_KEYWORDS[key];
  return Boolean(phrase && !isWeakAutoGeneratedPhrase(phrase, key));
}
...
export function resolveImageSearchKeyword(
  ...
  if (curated && !isWeakAutoGeneratedPhrase(curated, normalizedWord)) {
    return curated;
  }
  ...
  if (pos === "conjunction") return `fork road two paths decision choice sign`;
