/**
 * Build Jungle Jokers cast prompts for rank 3501–5000 (multi-cast policy).
 * Cast mix target: ~15% solo, ~45% duo, ~35% trio, ~5% all-four (+ group-word overrides).
 * Run: npm run build:jungle-casts-3501-5000
 */
...
function teachingProps(word: string): string {
  const curated = CURATED_IMAGE_KEYWORDS_RANK_3001_5000[word];
  if (curated) {
...
const header = readFileSync("src/data/jungle-cast-word-image-prompts.ts", "utf8")
  ...
  .replace(/rank 1–\d+(?:, \d+–\d+)*/, "rank 1–1000, 3000–5000");
