#!/usr/bin/env python3
"""Emit next N GenerateImage call JSON lines for jungle18 agent loops."""
...
if os.path.isdir(ART):
    try:
        EXISTING = {f for f in os.listdir(ART) if os.path.getsize(os.path.join(ART, f)) > 0}
    except Exception:
        pass
...
def main():
    if len(sys.argv) < 3:
        print("Usage: jungle18-next.py <from_batch_int> <to_batch_int> [count]", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Total pending: {len(pending)}", file=sys.stderr)
