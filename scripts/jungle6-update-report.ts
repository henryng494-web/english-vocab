#!/usr/bin/env npx tsx
/** Update cast-generation-report.json after jungle6 batch. */
...
if (words.length === 0) {
  console.error("Usage: jungle6-update-report.ts <word> [word...]");
  process.exit(1);
}
...
if (existsSync(REPORT)) {
  try {
    ...
    if (prev.bundle === "jungle6") report = { ...report, ...prev, okWords: prev.okWords ?? [] };
  } catch {
    /* fresh report */
  }
}
...
for (const word of words) {
  ...
  if (existsSync(jpg) && existsSync(jpg) && readFileSync(jpg).length > 10_000) {
    okSet.add(key);
    failSet.delete(key);
  } else {
    failSet.add(key);
...
console.log(`jungle6 report: ok=${report.ok} fail=${report.fail}`);
