#!/usr/bin/env python3
"""Queue + compact/sanitized prompts for jungle17 batches 060-069."""
import json
import os
import re
import sys
    ...
    for n in BATCHES:
        with open(f"/tmp/jungle17-batches/batch-{n:03d}.jsonl") as f:
            for line in f:
                if line.strip():
                    j = json.loads(line)
                    if not artifact_ok(j["filename"]) and j["word"] not in load_failed():
                        jobs.append(j)
    return jobs
...
if __name__ == "__main__":
    ...
    if cmd == "status":
        jobs = pending()
        print(json.dumps({"pending": len(jobs), "batches": "060-069"}))
    elif cmd == "next":
        ...
        sanitize = "--sanitize" in sys.argv
        jobs = [prepare_call(j, sanitize=sanitize) for j in pending()]
        print(json.dumps(jobs, ensure_ascii=False))
    else:
        print("usage: status|next [--sanitize]|export [--sanitize]", file=sys.stderr)
        sys.exit(1)
