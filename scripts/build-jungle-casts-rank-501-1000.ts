/**
 * Build Jungle Jokers cast prompts for rank 501–1000 and merge into existing entries.
 * Run: npm run build:jungle-casts-501-1000
 */
...
/** Collective / group words in 501–1000 → all four mascots. */
...
/** Interactive / duo words in 501–1000 → duo mascots. */
const CAST_DUO: Record<string, readonly JungleCastMember[]> = {
  marry: ["monkey", "elephant"],
  wedding: ["monkey", "elephant"],
  kiss: ["monkey", "tiger"],
  agree: ["elephant", "crocodile"],
  apologize: ["monkey", "elephant"],
  forgive: ["elephant", "monkey"],
  join: ["crocodile", "tiger"],
  share: ["monkey", "crocodile"],
  partner: ["elephant", "tiger"],
  enemy: ["tiger", "monkey"],
  visit: ["monkey", "elephant"],
  teach: ["elephant", "crocodile"],
  learn: ["monkey", "elephant"],
  study: ["crocodile", "tiger"],
  protect: ["elephant", "tiger"],
  attack: ["tiger", "monkey"],
  double: ["monkey", "elephant"],
  twice: ["crocodile", "tiger"],
};
...
function teachingProps(word: string): string {
  const curated = CURATED_IMAGE_KEYWORDS_RANK_501_1000[word];
  if (curated) {
