/**
 * Add missing preset headwords (rank ≤6000) into jungle cast prompts.
 * Run: npx tsx scripts/build-jungle-casts-missing.ts
 */
...
function assignCast(index: number): readonly JungleCastMember[] {
  const bucket = index % 20;
  if (bucket <= 2) return [MEMBERS[index % MEMBERS.length]!];
  if (bucket <= 11) return DUO_COMBOS[index % DUO_COMBOS.length]!;
  if (bucket <= 18) return TRIO_COMBOS[index % TRIO_COMBOS.length]!;
  return MEMBERS;
}

const presetWords = getWordsInRange(1, 6000).map((e) => e.word);
const missingWords = presetWords.filter((w) => !(w in JUNGLE_WORD_IMAGE_ENTRIES));

if (missingWords.length === 0) {
  console.log("No missing preset words in rank 1–6000.");
  process.exit(0);
}

const newEntries: Record<string, JungleWordImageEntry> = {};
missingWords.forEach((word, index) => {
  const cast = assignCast(index);
  let expressions: string;
  if (cast.length === 1) expressions = soloExpression(cast[0]!);
  else if (cast.length === 4) {
    expressions =
      "All four: expressive faces matching word meaning. Tiger: sphere only. Crocodile: log low. Elephant: stick arms visible. Monkey: two arms two legs only.";
  } else {
    expressions = multiExpression(cast);
  }
  newEntries[word] = { cast: [...cast], scene: buildScene(word, cast), expressions };
});

const merged = { ...JUNGLE_WORD_IMAGE_ENTRIES, ...newEntries };
const fullSource = readFileSync("src/data/jungle-cast-word-image-prompts.ts", "utf8");
const header = fullSource.split("export const JUNGLE_WORD_IMAGE_ENTRIES")[0];
...
console.log(`Added ${missingWords.length} missing prompts: ${missingWords.join(", ")}`);
