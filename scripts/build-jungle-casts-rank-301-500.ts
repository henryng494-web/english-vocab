/**
 * Build Jungle Jokers cast prompts for rank 301–500 and merge into existing entries.
 * Run: npm run build:jungle-casts-301-500
 */
...
const CAST_DUO: Record<string, readonly JungleCastMember[]> = {
  couple: ["monkey", "elephant"],
  husband: ["elephant", "tiger"],
  daughter: ["monkey", "crocodile"],
  sister: ["monkey", "tiger"],
  fight: ["tiger", "monkey"],
  dance: ["crocodile", "tiger"],
  welcome: ["monkey", "tiger"],
  trust: ["elephant", "monkey"],
  promise: ["elephant", "crocodile"],
...
function teachingProps(word: string): string {
  const curated = CURATED_IMAGE_KEYWORDS_RANK_301_500[word];
  if (curated) {
