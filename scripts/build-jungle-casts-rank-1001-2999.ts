/**
 * Build Jungle Jokers cast prompts for rank 1001–2999 (multi-cast policy).
 * Cast mix target: ~15% solo, ~45% duo, ~35% trio, ~10% all-four (+ group-word overrides).
 * Run: npm run build:jungle-casts-1001-2999
 */
...
const CAST_DUO: Record<string, readonly JungleCastMember[]> = {
  ally: ["monkey", "elephant"],
  argue: ["tiger", "monkey"],
  compete: ["tiger", "crocodile"],
  cooperate: ["elephant", "monkey"],
  debate: ["monkey", "tiger"],
  divorce: ["monkey", "elephant"],
  duel: ["tiger", "monkey"],
  marry: ["monkey", "elephant"],
  negotiate: ["elephant", "crocodile"],
  partner: ["elephant", "tiger"],
  rival: ["tiger", "monkey"],
  teammate: ["monkey", "crocodile"],
  boyfriend: ["monkey", "elephant"],
  girlfriend: ["monkey", "tiger"],
  husband: ["elephant", "monkey"],
  wife: ["monkey", "elephant"],
  enemy: ["tiger", "crocodile"],
  friend: ["monkey", "elephant"],
};

/** Small-group words → fixed trio. */
const CAST_TRIO: Record<string, readonly JungleCastMember[]> = {
  jury: ["monkey", "elephant", "tiger"],
  panel: ["elephant", "crocodile", "tiger"],
  trio: ["monkey", "elephant", "crocodile"],
  committee: ["monkey", "elephant", "crocodile"],
};
...
function teachingProps(word: string): string {
  const curated = CURATED_IMAGE_KEYWORDS_RANK_1001_3000[word];
  if (curated) {
...
/** ~15% solo, ~45% duo, ~35% trio, ~10% all-four (bucket rotation). */
function assignCast(word: string, index: number): readonly JungleCastMember[] {
  ...
  return MEMBERS; // 10% all-four rotation
}
...
const header = readFileSync("src/data/jungle-cast-word-image-prompts.ts", "utf8")
  ...
  .replace(/rank 1–\d+(?:, \d+–\d+)*/, "rank 1–2999, 3000–5000");
