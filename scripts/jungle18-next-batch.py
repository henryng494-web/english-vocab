...
def main():
    if len(sys.argv) < 3:
        print("Usage: jungle18-next-batch.py <from_batch_int> <to_batch_int> [limit]", file=sys.stderr)
        sys.exit(1)
    ...
    out_file = "/tmp/jungle18-next.json"
    data = []
    for j in selected:
        data.append({
            "description": desc(j),
    ...
    print(f"Pending total: {len(pending)}. Wrote next {len(data)} items to {out_file}.")
    for i, item in enumerate(data):
        print(f"[{i+1}/{len(data)}] {item['word']} ({item['filename']})")
