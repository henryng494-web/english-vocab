...
def main():
    b_from = int(sys.argv[1]) if len(sys.argv) > 1 else 60
    b_to = int(sys.argv[2]) if len(sys.argv) > 2 else 69
    limit = int(sys.argv[3]) if len(sys.argv) > 3 else 5
    out_file = sys.argv[4] if len(sys.argv) > 4 else "/tmp/next_items.json"
